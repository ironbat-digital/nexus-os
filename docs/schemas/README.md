...
La suite de validación de referencia (`test_managed_platform_schemas.py`) vive en la implementación
**upstream** (Console/Runtime) y **no** se incluye en este repositorio de docs y contratos. Comprueba
que (1) cada esquema (ambas versiones) es JSON Schema 2020-12 válido, (2) cada ejemplo valida contra su
esquema (resolviendo `$ref` cross-version por `$id`), (3) cada fixture negativo es rechazado, (4) ningún
ejemplo filtra secretos en texto plano, (5) se cumplen la forma del sobre de firma e invariantes críticas
de producto (edición/entitlement/suscripción/acceso a paquetes/modalidad) y (6) los enlaces relativos de
la documentación resuelven.
