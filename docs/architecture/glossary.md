...
| **PersonalStack (`personal-local-v1`)** | **Fuente de verdad máquina** del stack local **único** de Personal v1: Docker Compose + SQLite cifrado + loopback + conectores Anthropic/OpenAI por referencia. `spec.boundaries` fija en `false` las prohibiciones de Personal (arquitectura, cloud, modalidad, proveedor no-default, multiusuario, packs premium, facturación, flota gestionada). | `personal-stack.schema.json` |
| **AssistantHandoff** | Metadatos del handoff al asistente de código (`claude/openclaw/genérico`, rol `setup_executor`). Referencia el lock `PersonalStack`; el `SETUP.md` es un **render** (`is_source_of_truth: false`) y **nunca** contiene secretos en claro. | `assistant-handoff.schema.json` |
| **SecretBundleRef** | Referencia embebible al **bundle cifrado** (age/X25519) de secretos. Solo nombres lógicos y punteros; **prohíbe** por construcción cualquier valor, ciphertext o clave. | `secret-bundle-ref.schema.json` |
